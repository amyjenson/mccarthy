...
parser = ArgumentParser(
    description="""Solve 2D glacier Glen-Stokes problem.  Requires an activated
...
parser.add_argument(
    "-o", metavar="NAME", type=str, default="", help="output file name ending with .pvd"
)
parser.add_argument(
    "-osurface",
    metavar="NAME",
...
parser.add_argument(
    ...
    help="solver package from:\n%s\n(default=Direct)" % (",".join(packagechoices)),
)
parser.add_argument(
    "-refine",
    type=int,
...
parser.add_argument(
    "-slab", action="store_true", help="compute errors relative to slab-on-slope"
)
...
warnings.filterwarnings("ignore", message=".*pytools.persistent_dict.*")
...
def describe(thismesh, indent=0):
    if thismesh.comm.size == 1:
        printpar(
            "mesh has %d elements (cells) and %d vertices"
            % (thismesh.num_cells(), thismesh.num_vertices()),
            indent=indent,
        )
    else:
        PETSc.Sys.syncPrint(
            "rank %d owns %d elements (cells) and can access %d vertices"
...
mm = MomentumModel(
    eps=args.eps, alpha=args.alpha, Dtyp_pera=args.Dtyp, Hin=args.Hin, Hout=args.Hout
)
...
def momentumsolve(package=args.package, upold=None, upcoarse=None, indent=0):
    ...
    printpar(
        "flow speed: av = %10.3f m a-1,  max = %10.3f m a-1"
        % (secpera * umagav, secpera * umagmax),
        indent=indent + 1,
    )
    printpar(
        "pressure:   av = %10.3f bar,    max = %10.3f bar"
...
def numericalerrorsslab(indent=0):
    ...
    printpar(
        "numerical errors: |u-uex|_2/|uex|_2 = %.3e, |p-pex|_2/|pex|_2 = %.3e"
...
if args.sequence > 0:
    l = args.sequence
    printpar(
        f"solving for velocity and pressure using {l} levels of grid-sequencing ...",
        indent=l,
    )
...
if len(args.o) > 0:
    ...
    if mesh.comm.size > 1:
        ...
        VTKFile(args.o).write(u, p, d, nu, f, rank)
    else:
        ...
        VTKFile(args.o).write(u, p, d, nu, f, chi)
